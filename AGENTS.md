...
a **pasted offer / recruiter message**, or **a screenshot** of the chat (OCR'd locally via tesseract.js);
...
app/api/investigate/route.ts POST { input?, image? }: OCR image → scope guard → Wire evidence → DeepSeek verdict → fallback
...
- **DeepSeek** (`deepseek-chat`): use `response_format: { type: "json_object" }` for clean JSON.
  Screenshot OCR runs **locally** via tesseract.js (+ jimp dark-mode preprocessing) — DeepSeek has no
  vision input, so don't try to send it images.
